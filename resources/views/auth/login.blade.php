              ...
              <div class="col-lg-4 mx-auto">
                <div class="auth-form-light text-left p-5">
                  ...
                  <form method="POST" action="{{ route('login') }}">
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            <input id="password" type="password" class="form-control @error('password') is-invalid @enderror" placeholder="Masukkan Password" name="password"  autocomplete="current-password">

                            @error('password')
                                <span class="text-danger">
                                    <sub>{{ "Password Kosong atau Salah" }}</sub>
                                </span>
                            @enderror
                        </div>
                    </div>
                    ...
                    <div class="my-2 d-flex justify-content-between align-items-center">
                        <div class="form-check">
                            <label class="form-check-label" for="remember">
                                {{ __('Ingat Saya') }}
                                <input class="form-check-input" type="checkbox" name="remember" id="remember" {{ old('remember') ? 'checked' : '' }}> </label>
                        </div>
                        @if (Route::has('password.request'))
                        <a class="auth-link text-black float-end" href="{{ route('password.request') }}">
                            {{ __('Lupa Password?') }}
                        </a>
                    @endif
                      </div>
