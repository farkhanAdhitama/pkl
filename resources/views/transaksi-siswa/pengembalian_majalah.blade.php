...
@section('content')
    {{-- Notif  Pengembalian Majalah berhasil dihapus --}}
    @if ($message = Session::get('deletesuccess_majalah'))
        <script>
            Swal.fire(
                'Berhasil!',
                'Data Pengembalian Majalah Berhasil Dihapus!',
                'success'
            )
        </script>
    @endif

    {{-- Notif  Pengembalian Majalah berhasil dihapus semua --}}
    @if ($message = Session::get('deletesuccess_majalah_all'))
        <script>
            Swal.fire(
                'Berhasil!',
                'Semua Data Pengembalian Majalah Berhasil Dihapus!',
                'success'
            )
        </script>
    @endif
    <div class="page-header">
        <h3 class="page-title">
            ...
            </span> Riwayat Pengembalian Majalah Siswa
        </h3>
        <div class="mb-3 d-flex justify-content-end">
            <a href="/exportpdf_pengembalian_majalah"><button type="button" class="btn btn-sm btn-info btn-icon-text me-1">
                    <i class="mdi mdi-printer btn-icon-append"></i> Cetak PDF </button></a>
            <a href="/exportexcel_pengembalian_majalah"> <button type="button"
    ...
    <script>
        $('.delete').click(function() {
            ...
            Swal.fire({
                title: 'Apakah Yakin?',
                text: "Hapus Pengembalian  " + majalah + " Oleh " + anggota + "",
                icon: 'warning',
        ...
        $('.hapus_all').click(function() {
            Swal.fire({
                title: 'Apakah Yakin?',
                text: "Hapus Semua Data Pengembalian Majalah",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    window.location = "/deletePengembalianMajalahSiswaAll"
